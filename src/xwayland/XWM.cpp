...
void CXWM::sendIncrChunk(SXSelection& sel, SXTransfer& transfer) {
    ...
    Debug::log(LOG, "[xwm] Sending INCR chunk: offset={}, chunk={}, remaining={}", transfer.propertyStart, chunkSize, remaining);

    if (chunkSize > 0) {
        // Send the next chunk of data
        xcb_change_property(conn, XCB_PROP_MODE_REPLACE, transfer.request.requestor, transfer.request.property, transfer.request.target, 8, chunkSize,
                            transfer.data.data() + transfer.propertyStart);
        ...
        xcb_change_property(conn, XCB_PROP_MODE_REPLACE, transfer.request.requestor, transfer.request.property, transfer.request.target, 8, 0, nullptr);
        xcb_flush(conn);

        // Clean up the completed transfer
        auto it = std::ranges::find_if(sel.transfers, [&transfer](const auto& t) { return t.get() == &transfer; });
        if (it != sel.transfers.end()) {
            sel.transfers.erase(it);
        }
    }
...
bool CXWM::handleSelectionPropertyNotify(xcb_property_notify_event_t* e) {
    ...
    for (auto* sel : {&m_clipboard, &m_primarySelection}) {
        auto it =
            std::ranges::find_if(sel->transfers, [e](const auto& t) { return t->incremental && t->out && t->request.requestor == e->window && t->request.property == e->atom; });
